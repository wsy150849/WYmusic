...
export default {
  ...
  computed: {
    ...
    detailPlayerFlag() {
      return this.$store.getters.detailPlayerFlag;
    },
    isPlays() {
    ...
    songLrc() {
      ...
      if (this.audios.lrc) {
        const lrcSentenceRE = /^\[(\d{1,3}):(\d{2}\.\d{3})\](.*)/;

        const lrcItems = this.audio.lrc
          .split("\n")
          .filter(item => {
    ...
    lrcOffset() {
    //   console.log(this.songLrc[this.songLrc.length - 1]);
      if (this.songLrc) {
        const songLength = this.songLrc[this.songLrc.length - 1].seconds;
        const audio = { songLength };
        this.$store.commit("setAudio", audio);
        const offset =
          (this.songLrc.length -
            document.querySelectorAll(".wait-lrc").length -
            2) *
          -20;
